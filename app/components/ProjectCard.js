...
const cardInner = (title, description, tags, contentVisible, contentPresent) => (
  <div className="flex h-full flex-col rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm transition-all duration-300 group-hover:-translate-y-1 group-hover:border-violet-400 group-hover:shadow-[0_0_0_1px_theme(colors.violet.400),0_8px_24px_-4px_theme(colors.violet.200)] dark:border-zinc-800 dark:bg-zinc-900 dark:group-hover:border-violet-500 dark:group-hover:shadow-[0_0_0_1px_theme(colors.violet.500),0_8px_24px_-4px_theme(colors.violet.950)]">
  ...
  if (onClick) {
    return (
      <button onClick={onClick} className="group block h-full w-full text-left">
        {cardInner(title, description, tags, contentVisible, contentPresent)}
      </button>
    );
  }
